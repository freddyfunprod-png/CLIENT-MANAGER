...
async def mark_contacted(data: dict):
    """Mark 'Primer contacto realizado' and optionally assign vendor for a list of client IDs."""
    client_ids = data.get("client_ids", [])
    vendor_name = data.get("vendor_name")  # optional
    if not client_ids:
        return {"ok": False}
    async with aiosqlite.connect(DB_PATH) as db:
